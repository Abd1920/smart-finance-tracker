...
const DebtForm = ({ isOpen, onClose, onSubmit, debt, isLoading }) => {
  const isEdit = !!debt;
  ...
  useEffect(() => {
    if (debt) {
      ...
      setForm({ debtType: 'to_pay', personName: '', amount: '', description: '', dueDate: '' });
    }
    setErrors({});
  }, [debt, isOpen]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEdit ? 'Edit Debt' : 'Add Debt / Loan'}>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="label">
            {form.debtType === 'to_pay' ? 'Creditor Name' : 'Debtor Name'}
          </label>
          <input
            name="personName"
            value={form.personName}
            onChange={handleChange}
            placeholder={form.debtType === 'to_pay' ? 'Who do you owe?' : 'Who owes you?'}
